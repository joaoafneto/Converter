...
public class XmlReader {
    private StateType parseStateType(Element parentElement) {
        boolean isInitial = getInitialNode(parentElement);

        boolean isFinal = getFinalNode(parentElement);

        return isInitial && isFinal ? StateType.Both
                : isInitial ? StateType.Initial : isFinal ? StateType.Final : StateType.Default;
    }

    private boolean getFinalNode(Element parentElement) {
        return StreamSupport.stream(XmlHelper.iterable(parentElement.getChildNodes()).spliterator(), false)
                .anyMatch(childElement -> childElement.getNodeType() == Node.ELEMENT_NODE
                        && ((Element) childElement).getTagName() == "final");
    }

    private boolean getInitialNode(Element parentElement) {
        return StreamSupport.stream(XmlHelper.iterable(parentElement.getChildNodes()).spliterator(), false)
                .anyMatch(childElement -> childElement.getNodeType() == Node.ELEMENT_NODE
    ...
    public Automaton readAutomatonFromFile(String filename) {
        try {
